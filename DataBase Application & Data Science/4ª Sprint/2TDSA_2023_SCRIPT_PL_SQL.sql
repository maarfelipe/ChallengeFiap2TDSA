...
CREATE OR REPLACE PACKAGE BODY PackageCadastroFunc IS
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            INSERT INTO t_aisb_erro (cd_erro, nm_erro, dt_ocorrencia, nm_usuario)
            VALUES (v_cd_erro, 'Erro ao cadastrar funcionário', SYSDATE, USER);
...
---------------------------------------------------------------------------
--Package da Função que faz a verificação ao cadastrar um usuário, em que, caso esse
...
---------------------------------------------------------------------
--Package de Procedure que irá exibir um relatório de todos os parceiros que encerram 
...
CREATE OR REPLACE PACKAGE BODY PackageRelatorioParceiros IS
    ...
    IS
        ...
        v_relatorio VARCHAR2(4000) := 'Relatório de Parceiros Encerrados' || CHR(10) || '--------------------------------' || CHR(10);
    BEGIN
        BEGIN
            ...
            v_relatorio := v_relatorio || '--------------------------------' || CHR(10) || 'Fim do Relatório' || CHR(10);
        EXCEPTION
            WHEN OTHERS THEN
                v_cd_erro := -1; 
                v_nm_erro := SQLERRM;
                ...
                v_relatorio := 'Ocorreu um erro durante a execução da função.' || CHR(10) || 'Detalhes do erro: ' || v_nm_erro || CHR(10);
        END;
